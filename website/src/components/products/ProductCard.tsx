...
const ProductCard = memo(({ product }: { product: any }) => {
  ...
  return (
    ...
    >
      {/* IMAGE CONTAINER */}
     <div className="relative aspect-square w-full bg-[#f8fafc] rounded-xl sm:rounded-2xl p-2 mb-2 sm:mb-3 flex items-center justify-center overflow-hidden">

  {/* STATUS BADGE */}
  <div className="absolute top-1 left-1 z-20">
    {isOutOfStock ? (
      <span className="bg-red-500 text-white text-[8px] px-2 py-0.5 rounded-md font-bold">
        OUT OF STOCK
      </span>
    ) : product.stock <= 10 ? (
      <span className="bg-orange-400 text-white text-[8px] px-2 py-0.5 rounded-md font-bold">
        LOW STOCK
      </span>
    ) : null}
  </div>
          ...
          {quantity === 0 ? (
            <button
              ...
              className={`${isOutOfStock
                ...
                } text-white w-7 h-7 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center shadow-md active:scale-90 transition-all`}
